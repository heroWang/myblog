...
from roll_blog.text.link import media_link, absolute

class Media(object):
  """Handle XML-RPC calls related to images and other media.

  Mixin for roll_blog.api.handlers.APIHandler.
  """
  @rpc
  @coroutine
  def metaWeblog_newMediaObject(self,blogid,user,password,struct):
    name = struct['name']
    content = struct['bits'].data
  ...
  def store_image(self,name,content,content_type):
    ...
    #This is the tail end of the URL,like 2012/06/06/foo.png.
    now = datetime.datetime.utcnow()
    fullname = media_link(now.year,now.month,now.day,name)
